...
#model = word2vec.setMinCount(5).setVectorSize(10).setSeed(2022).fit(inp)
#model = word2vec.setVectorSize(10).setSeed(2022).fit(inp)
model = word2vec.setLearningRate(0.01).setWindowSize(4).setMinCount(5).setVectorSize(20).setSeed(2022).fit(training_data)
...
#print vec
test_data = [ "furniture", "shaver", "sofa", "toddler", "sport","xbox", "led","organizer"]
for w in test_data:
...
model.save(sc, "../data/model/word2vec_0711")
sc.stop()
